        ...
        const slotItems = ["ga", "tom", "bau", "cua", "ca", "nai"];
            const moneyUser = (await Currencies.getData(event.senderID)).money;
                var moneyBet = parseInt(args[1]);
                    if (!args[0] || !isNaN(args[0])) return api.sendMessage("[𝙉𝙑𝙃🐧] => Hãy Bấm : /baucua [bầu/cua/cá/nai/gà/tôm] [số tiền]",event.threadID, event.messageID);
                    if (isNaN(moneyBet) || moneyBet <= 0) return api.sendMessage("[𝙉𝙑𝙃🐧] => Số tiền đặt cược không được để trống hoặc là số tiền âm", event.threadID, event.messageID);
                if (moneyBet > moneyUser) return api.sendMessage("[𝙉𝙑𝙃🐧] => Số tiền bạn đặt lớn hơn số dư của bạn!", event.threadID, event.messageID);
            if (moneyBet < 50) return api.sendMessage("[𝙉𝙑𝙃🐧] => Số tiền đặt không được dưới 50 đô!", event.threadID, event.messageID);
                ...
                switch (args[0]) {
                    ...
                    case "tôm":
                        case "Tôm": itemm = "tom";
                                icon = '🦞';
                            break;
                                default: return api.sendMessage("[𝙉𝙑𝙃🐧] => Hãy Bấm : /baucua [bầu/cua/cá/nai/gà/tôm] [số tiền] biết chưa đồ ngốc",event.threadID, event.messageID);
                }      
                await get(number[0],number[1],number[2]);
            api.sendMessage({body:"[𝙉𝙑𝙃🐧] => Đang Đập, À Không Đang Lắc Để Cho Mấy Con Nghiện chơi!",attachment: createReadStream(__dirname + "/cache/baucua.gif")},event.threadID,async (error,info) => {
