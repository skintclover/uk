...
module.exports.handleReply = async function({ api, event, handleReply, getText }) {
    ...
    if (isNaN(body) && body.indexOf("c") == 0 || body.indexOf("cancel") == 0) {
        ...
        for (const singleIndex of index) {
            ...
            api.changeNickname(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "𝑱𝒂𝒚" : global.config.BOTNAME}`, handleReply.pending[singleIndex - 1].threadID, api.getCurrentUserID());
...
[🌐] 𝗙𝗯 𝗮𝗱𝗺𝗶𝗻: https://www.facebook.com/1712332674`, attachment: fs.createReadStream(__dirname + "/cache/joinbox/join.mp4")} ,handleReply.pending[singleIndex - 1].threadID));
            count+=1;
