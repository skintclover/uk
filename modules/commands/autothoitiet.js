...
msg = msg.replace(/{hours}/g, hours)
  ...
  msg = msg.replace(/{seconds}/g, seconds)
    msg = msg.replace(/{time}/g, require("moment-timezone").tz("Asia/Ho_Chi_Minh").format("HH:mm:ss (D/MM/YYYY) (dddd)")).replace(/{thinh}/g, (await axios.get(`https://apirandom.dungkonuwu.repl.co/thinh`)).data.data)
            msg = {
                body: msg, attachment: (await axios.get((await axios.get(`https://anime4k.studentisfuture.repl.co/anime`)).data.data, {
                    responseType: 'stream'
                })).data
            };
