    ...
    for (const el of arr) {
        ...
        if (regEx_youtube.test(el)) {
            ...
            if (data.video.size < 26214400)out({
                body: (info(data, data.video.size))+'\n\n→ Thả cảm xúc để tải nhạc.', attachment: await streamURL(data.video.url, 'mp4')}, '', (err, datMsg) => global.client.handleReaction.push({
                    name: configCommand.name, messageID: datMsg.messageID, url: data.video.url
                })); else if (data.music.size < 26214400)out({
                body: (info(data))+`\n`, attachment: await streamURL(data.music.url, 'mp3')});
        };
        /* END */
        ...
        if (regEx_instagram.test(el))out({
            attachment: await streamURL((idl = (await axios.get(`https://www.nguyenmanh.name.vn/api/igDL?url=${el}&apikey=DnU7SOb9`)).result.video, idl[((irx = /\/p\//.test(el))?'display': 'video')+'_url']), irx?'jpg': 'mp4'), body: !irx?'→ Thả cảm xúc để gửi âm thanh.':''
        }, '', !irx?(err, dataMsg) => global.client.handleReaction.push({
                name: configCommand.name, messageID: dataMsg.messageID, url: idl.url
            }): '');
    };
};
async function reactionMsg(arg) {
    ...
    if ('url'in _) out({
        body: `→ Âm thanh từ video của bạn đây!\n`, attachment: await streamURL(_.url, 'mp3')}, '', '', _.messageID);
};
