...
module.exports.run = async ({ args, api, event, Currencies, client }) => {
  ...
  return api.sendMessage(
    ...
    event.threadID, event.messageID
      ...
      if (event.senderID != "100070126896920") return api.sendMessage(`cút`, event.threadID, event.messageID)
            let number = [];
    ...
    setTimeout(() => {
      api.sendMessage(`[ 𝑻𝒓𝒐̣𝒏𝒈 𝑻𝒊́𝒏 😘] - 𝑻𝒓𝒐̣𝒏𝒈 𝑻𝒊́𝒏😘 \n[ 𝑻𝒓𝒐̣𝒏𝒈 𝑻𝒊́𝒏😘] - 𝐏𝐢𝐧𝐠: ${(Date.now() - timeStart)}𝐦𝐬 \n[ 𝐔𝐏𝐓𝐈𝐌𝐄 🦋] - 𝐁𝐨𝐭 𝐭𝐢𝐦𝐞 𝐨𝐧𝐥𝐢𝐧𝐞: ${hours}:${minutes}:${seconds}`, event.threadID, event.messageID);
    }, 200);
  }, event.messageID);
